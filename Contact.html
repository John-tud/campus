<!DOCTYPE html>
<html>
  <head>
    <button type="button">Click Me!</button>
    
  </head>

  <body>
        <h2>Contact Page</h2><hr>
    <p>If you have any queries or would like to contact us you must complete the form below before you can contact us</p>
  </section>
    
    </details>
    <hr>
  <ul>
    <p><b>Contacts</b></p>
    <li>John: <span class="tel">086546652</span></li>
    <li>Jacob: <span class="tel">086546652</span></li>
  <p><b>Email: </b>Wheelsonfire@hotmail.ie</p>
  <hr>

  Completion Progress:
  <progress value="2" max="9"></progress>
  <hr>
  <form>  
     Name: <input type="text"><br>
   Email: <input type="text"><br>
   DOB: <input type="text">
    </fieldset>
</form> 
  </body>
  <br>
  <footer>
      <address>
  Written by <a href="wheelsonfire@tudublin.ie">Wheels on Fire</a>.<br>
  Visit us at:<br>
         <a href="wheelsonfire.com"> Wheels on Fire</a>
</address>

<label for="feedback">Share some feedback about our website:</label>
<textarea id="Feedback" name="Feedback" rows="4" cols="33"></textarea>
    </footer>
</html>
